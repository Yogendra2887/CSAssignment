package com.github.cs.loganalyser.validator;
...
public class LogAnalyserValidator {
    ...
    public void validateInput(Context context, String... args) {
        ...
        //if path does not exist then it is application flow
        //if path exists then it is JUNIT flow
        if (null == path) {
            validateArguments(args);
            validateFilePath(context, args[0]);
        } else {
            validateFilePath(context, path);
        }
    }
